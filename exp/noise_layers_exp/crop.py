...
def get_random_rectangle_inside(image, crop_scale):
    """
    ...
    remaining_height = int(np.rint(crop_scale * image_height))
    remaining_width = int(np.rint(crop_scale * image_width))
...
class Crop(nn.Module):
    ...
    def forward(self, image):
        ...
        # device = image.device
        h_start, h_end, w_start, w_end = get_random_rectangle_inside(image, self.crop_scale)

        ones = torch.zeros_like(image)

        ones[:, :, h_start: h_end, w_start: w_end] = 1
